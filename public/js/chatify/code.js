  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".messenger-infoView")
          .find(".avatar")
          .css("background-image", 'url("/import/profileImg/' + data.fetch.profile + '")');
        $(".header-avatar").css(
          "background-image",
          'url("/import/profileImg/' + data.fetch.profile + '")'
        );
  ...
  if (inputValue.length > 0 || hasFile) {
    ...
    $.ajax({
      ...
      error: () => {
        ...
        // error log
        console.error(
          "Échec de l'envoi du message ! Veuillez vérifier la réponse de votre serveur."
        );
      },
    });
  }
